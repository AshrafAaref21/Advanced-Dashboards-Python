...
def run():
    ...
    # st.set_page_config(
    #     page_title="Seer-Dashboard",
    #     page_icon=logo,
    #     layout="wide",
    #     initial_sidebar_state="expanded")
    link = '[![portfolio](https://img.shields.io/badge/my_portfolio-000?style=for-the-badge&logo=ko-fi&logoColor=white)](https://www.fiverr.com/ashrafaaref?public_mode=true/)'
...
historic_district_cases_df['All Berlin'] = historic_district_cases_df.drop('Datum', axis=1).sum(
    axis=1)
print(historic_district_cases_df)
...
districts = ['Lichtenberg', 'All Berlin', 'Mitte', 'Charlottenburg-Wilmersdorf', 'Friedrichshain-Kreuzberg', 'Neukoelln',
             'Tempelhof-Schoeneberg', 'Pankow', 'Reinickendorf', 'Steglitz-Zehlendorf', 'Spandau', 'Marzahn-Hellersdorf', 'Treptow-Koepenick']
populations = [2.91452, 37.54418, 3.84172, 3.42332, 2.89762, 3.29691,
               3.51644, 4.07765, 2.65225, 3.08697, 2.43977, 2.68548, 2.71153]
...
for i in selected_districts:
    ...
    data_to_plot = pd.concat(
        [data_to_plot, historic_cases[new_col_name]], axis=1)


# Creating a 7 day rolling sum of cases per district
for i in selected_districts:
    new_reported_cases['Seven Day Sum for %s' %
                       i] = new_reported_cases[i].rolling(7).sum()
...
for i in selected_districts:
    ...
    new_reported_cases['Seven Day Incidence for %s' %
                       i] = new_reported_cases['Seven Day Sum for %s' % i] / popn
...
for i in selected_districts:  # looping to plot each district
    plt.plot(incidence_data['Datum'],
             incidence_data['Seven Day Incidence for %s' % i])
...
plt.yticks(color='1')
plt.ylim((0))
plt.title('Seven Day Incidence - Last ' +
          str(days_to_show) + ' Days', color='1')
...
plt.xticks(rotation=45,
           ...
           color='1')
plt.yticks(color='1')
plt.title('Rolling 7-day-average - Last ' +
          str(days_to_show) + ' Days', color='1')
...
plt.xticks(rotation=45,
           ...
           color='1')
plt.yticks(color='1')
plt.title('New Reported Cases - Last ' +
          str(days_to_show) + ' Days', color='1')
...
st.write('''
    Dashboard created by [Eng Aaref](https://github.com/AshrafAaref21), with [Streamlit](https://www.streamlit.io).
